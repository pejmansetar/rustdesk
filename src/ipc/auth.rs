...
mod tests {
    ...
    fn test_executable_paths_match_windows_normalization() {
        let left = std::path::PathBuf::from(r"\\?\C:\Program Files\Remotik\remotik.exe");
        let right = std::path::PathBuf::from(r"c:\program files\remotik\remotik.exe");
    ...
    fn test_os_str_eq_ignore_ascii_case_for_process_names() {
        assert!(super::os_str_eq_ignore_ascii_case(
            Some(std::ffi::OsStr::new("Remotik")),
            Some(std::ffi::OsStr::new("remotik"))
        ));
        assert!(!super::os_str_eq_ignore_ascii_case(
            Some(std::ffi::OsStr::new("Remotik")),
    ...
    fn test_portable_service_helper_trust_requires_content_match() {
        let unique = format!(
            "remotik-portable-helper-trust-test-{}-{}",
            std::process::id(),
    ...
    fn test_portable_service_helper_trust_accepts_matching_content() {
        let unique = format!(
            "remotik-portable-helper-trust-match-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
